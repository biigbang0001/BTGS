...
function(add_windows_deploy_target)
  if(MINGW AND TARGET bitcoin AND TARGET bitcoin-qt AND TARGET bitcoind AND TARGET bitcoin-cli AND TARGET bitcoin-tx AND TARGET bitcoin-wallet AND TARGET bitcoin-util)
    ...
    generate_setup_nsi()
    add_custom_command(
      OUTPUT ${PROJECT_BINARY_DIR}/btgs-win64-setup.exe
      COMMAND ${CMAKE_COMMAND} -E make_directory ${PROJECT_BINARY_DIR}/release
      COMMAND ${CMAKE_STRIP} $<TARGET_FILE:bitcoin> -o ${PROJECT_BINARY_DIR}/release/btgs${CMAKE_EXECUTABLE_SUFFIX}
      COMMAND ${CMAKE_STRIP} $<TARGET_FILE:bitcoin-qt> -o ${PROJECT_BINARY_DIR}/release/btgs-qt${CMAKE_EXECUTABLE_SUFFIX}
      COMMAND ${CMAKE_STRIP} $<TARGET_FILE:bitcoind> -o ${PROJECT_BINARY_DIR}/release/btgsd${CMAKE_EXECUTABLE_SUFFIX}
      COMMAND ${CMAKE_STRIP} $<TARGET_FILE:bitcoin-cli> -o ${PROJECT_BINARY_DIR}/release/btgs-cli${CMAKE_EXECUTABLE_SUFFIX}
      COMMAND ${CMAKE_STRIP} $<TARGET_FILE:bitcoin-tx> -o ${PROJECT_BINARY_DIR}/release/btgs-tx${CMAKE_EXECUTABLE_SUFFIX}
      COMMAND ${CMAKE_STRIP} $<TARGET_FILE:bitcoin-wallet> -o ${PROJECT_BINARY_DIR}/release/btgs-wallet${CMAKE_EXECUTABLE_SUFFIX}
      COMMAND ${CMAKE_STRIP} $<TARGET_FILE:bitcoin-util> -o ${PROJECT_BINARY_DIR}/release/btgs-util${CMAKE_EXECUTABLE_SUFFIX}
      COMMAND ${MAKENSIS_EXECUTABLE} -V2 ${PROJECT_BINARY_DIR}/btgs-win64-setup.nsi
      DEPENDS bitcoin bitcoin-qt bitcoind bitcoin-cli bitcoin-tx bitcoin-wallet bitcoin-util
      VERBATIM
    )
    add_custom_target(deploy DEPENDS ${PROJECT_BINARY_DIR}/btgs-win64-setup.exe)
  endif()
endfunction()
